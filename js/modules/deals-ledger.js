...
function renderDeals() {
  ...
  if (dealRows.length && !display.length) {
    body.innerHTML = `<tr><td colspan="6" class="tone-dim ledger-empty-cell">No ${DEAL_STATUS_BY_KEY[statusFilter]?.label.toLowerCase() ?? ''} deals.</td></tr>`;
  ...
  document.getElementById('ledger-body').innerHTML = sorted.map((t) => {
    ...
    return `
      <tr class="rowlink" data-id="${esc(t.id)}" tabindex="0">
        <td class="tone-dim nowrap">${fmtDate(t.occurred_at)}</td>
        <td>
